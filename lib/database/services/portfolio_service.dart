...
class PortfolioService {
  ...
  // Получение элементов портфолио по ID фотографа
  Future<List<PortfolioItem>> getPortfolioByPhotographerId(int photographerId) async {
    try {
      final response = await supabase
          .from('portfolio_items')
          .select()
  ...
  Future<void> _loadAdditionalInfo(List<PortfolioItem> items) async {
    ...
    try {
      ...
      Set<int?> moodIds = items.map((item) => item.moodId).where((id) => id != null).cast<int>().toSet();
      Set<int?> locationIds = items.map((item) => item.locationId).where((id) => id != null).cast<int>().toSet();
